...
def train_net(net,epochs=100,batch_size=1,lr=0.0001,val_percent=0.05,save_cp=True,gpu=True,set_height=256,set_width=256):
    ...
    print('Training parameters(cos):Epochs:{} Batch size:{} Learning rate:{} Training size:{} validation size:{}'.format(epochs,batch_size,lr,len(id_dataset['train']),len(id_dataset['val'])))
    ...
    for epoch in range(epochs):
        ...
        train_imgs = get_images(train_data,dir_img,dir_mask,set_height,set_width)
        ...
        val_dice = eval_net(net,val_imgs)
        ...
        if max_val_dice < val_dice and save_cp == True:
            ...
            torch.save(net.state_dict(),dir_checkpoint+'unet.pth')
...
if __name__=='__main__':
    net=UNet(n_channels=1,n_classes=1,TransConv=True)
    ...
    try:
        train_net(net=net,epochs=epochs,batch_size=batch_size,lr=lr,val_percent=0.05,save_cp=True,gpu=True,set_height=set_height,set_width=set_width)
